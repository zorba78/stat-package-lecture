...
Chunk 옵션 & Default & 설명\\
\midrule
\rowcolor{gray!6}  eval & TRUE & R 실행 결과에 대응하는 코드 출력 여부\\
...
\item
\item
  \texttt{echo}: R 실행 결과에 대응하는 코드 출력 여부 (\texttt{TRUE/FALSE}, \texttt{default\ =\ TRUE})
\item
  \texttt{eval}: chunk 안에 작성한 스크립트(코드)의 실행 여부(\texttt{TRUE/FALSE}, \texttt{default\ =\ TRUE})
\item
  \texttt{include}: chunk 출력물을 출력 문서에 포함할지 여부 결정(\texttt{TRUE/FALSE}, \texttt{default\ =\ TRUE})
\item
  \texttt{results}:
\item
  \texttt{error}:
